...
void FbxParts::InitVertex(fbxsdk::FbxMesh * mesh)
{
	...
	for (DWORD poly = 0; poly < polygonCount_; poly++)
	{
		...
		for (int vertex = 0; vertex < 3; vertex++)
		{
			...
			pVertexData_[mesh->GetPolygonVertices()[startIndex + vertex]].tangent = XMVectorSet((float)tangent[0], (float)tangent[1], (float)tangent[2], 0.0f);
		}
	}
...
void FbxParts::InitTexture(fbxsdk::FbxSurfaceMaterial * pMaterial, const DWORD &i)
{
	...
	//ノーマルテクスチャ
	{
		...
		if (fileTextureCount > 0)
		{
			...
			//ファイルからテクスチャ作成
			pMaterial_[i].pNormalTex = new Texture;
			pMaterial_[i].pNormalTex->Load(name);
		}
		//画像が使われていない
		//テクスチャ無し
		//マテリアルはあるのでマテリアルの色をとってきたい
		else
		{
			pMaterial_[i].pNormalTex = nullptr;
		}
	}
}
