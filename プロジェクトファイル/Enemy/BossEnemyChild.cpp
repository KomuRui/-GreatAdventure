...
//コンストラクタ
BossEnemyChild::BossEnemyChild(GameObject* parent, std::string modelPath, std::string name)
...
void BossEnemyChild::EnemyChildStartUpdate()
{
	...
	ChildBossEnemyChildStartUpdate();
}

//更新
void BossEnemyChild::EnemyChildUpdate()
{
	//コライダーのポジション変更
	SetPosCollider(XMFLOAT3(ZERO, XMVectorGetY(XMVector3Normalize(vNormal_)), ZERO));

	//継承先用の更新
	ChildBossEnemyChildUpdate();
}

//ノックバックして死亡
void BossEnemyChild::KnockBackDie()
{
	//ノックバックしていないのなら
	if (!isKnockBack_)
	{
...
void BossEnemyChild::TimeMethod()
{
	Enter();
}

//何かのオブジェクトに当たった時に呼ばれる関数
void BossEnemyChild::OnCollision(GameObject* pTarget)
{
	//もしPlayerと当たったら
