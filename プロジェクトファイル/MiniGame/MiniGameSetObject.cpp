...
namespace
{
    ...
    static const int   MIN_GENERATING_TYPE = 0;   //最小生成の種類
    static const int   MAX_GENERATING_TYPE = 2;   //最大生成の種類
...
//真ん中
void MiniGameSetObject::Center() { Instantiate_Center<Pipe>(transform_.position_); }
...
void MiniGameSetObject::MultipleEdge() {
    ...
    Instantiate_Multiple_Edge<MiniGamePigEnemy>(pos); 
}
...
void MiniGameSetObject::Edge(){
    ...
    (Random(1, 2) == 1) ? Instantiate_Edge<MiniGameCoin>(pos)
                        : Instantiate_Edge<TurnCircle>(pos);
}
